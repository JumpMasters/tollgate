...
an `Idempotency-Key` on every mutation, most-restrictive denial naming the
binding node, and a thin FastAPI `api/` layer — but deliberately leaves the
wire contract open: URL shape, HTTP methods, status codes, header syntax,
and body schemas. The command routes need those pinned, and they become hard
to change the moment an SDK or any external caller depends on them.
...
- **RPC-style command paths under a version prefix:** `POST /v1/reserve`,
  ...
  keeps idempotency in the command layer, where it already lives.
...
- **Bearer credentials per RFC 6750:** `Authorization: Bearer <token>`.
  ...
  identical body, giving probes no distinguishing signal.
- **`Idempotency-Key` is a required request header** on reserve, commit,
  cancel, and grace-backfill (Stripe-style). Extend takes none — it
  is naturally idempotent; an Idempotency-Key sent to extend is
...
- **Error envelope** `{"error": {"code": "<snake_case>", "message": "…"}}`
  ...
  binding node on a denial); `code` is the stable programmatic key.
- **Typed-error to status mapping** (unmapped `TollgateError` subtypes fail
  ...
  key (denials are never cached), unlike the validation rejections.
  `BudgetNotFound` is 403 because default-deny is a policy outcome,
...
- **Grace backfill is a first-class command route.** The handler exists
  (ADR 0030) and the SDK's reconnection path needs an HTTP target;
...
- Identical 401s and identical unknown/foreign 403s keep the
...
- The response stored for idempotent replay is the handler-level
