...
`budget_balance` already aggregates everything at or below it, so a
...
authorization runs in the inverse direction from commands, and the
...
- **Optional `?scope=<kind>:<id>` filter** re-roots the returned subtree at a
  ...
  existence leak, mirroring ADR 0031's unknown/foreign `403`. A
...
- **Bearer credential required**, identical to the command routes: a missing,
  ...
  `WWW-Authenticate: Bearer`. Reads carry a credential like every
  command.
- **Utilization is reserved-inclusive:** `(reserved + committed + overage) /
  limit`, matching the `remaining` quantity the reserve guard gates on. It may
  exceed 100 under audited overage and is reported, not clamped. A
...
- The subtree listing doubles as the by-scope rollup, so no ledger read is
  ...
  oracle" holds.
- Reusing `authorizes` keeps one authorization definition for
