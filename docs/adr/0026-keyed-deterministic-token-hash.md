...
Authentication (ADR 0015) hashes the presented bearer token and **looks up**
an active `api_credential` by that hash; the token itself is never stored. The
schema makes `api_credential.token_hash` `UNIQUE`, and the lookup is a single
...
Password hashes such as `argon2`/`bcrypt` exist for credential tokens, but those:
...
This refines the earlier `argon2`/`bcrypt` guidance for the token case; it **complements**
...
- The schema's "salt" is the shared server pepper, not a per-row salt; deterministic
