...
Under opt-in grace (§5.6) the SDK may dispatch calls while enforcement is
...
caller-asserted amount, matching the commit trust model of §5.0). The handler
...
against — the mirror of §5.3's default-deny.
...
- The per-budget grace *cap* is tracked and enforced SDK-side during the outage
  (plan 14); the `budget` schema carries no grace column in V1, so the server does
